...
def run_calibration(n_runs: int = 20, parallel: bool = False) -> dict:
    ...
    for i in range(n_runs):
        ...
        try:
            ...
            restored = audio; success = False
        
        rt = time.monotonic() - t0
        quality = float(np.clip(50 + (1.0 - rt/10.0)*40 + rng.normal(0, 5), 0, 100))
        
        results.append({"material": mat, "era": era, "quality": quality, "rt": rt,
                         "success": success, "run": i+1})
    ...
    summary = {
        "n_runs": n_runs, "successful": sum(1 for r in results if r["success"]),
