...
def mag_pha_istft(mag, pha, n_fft=N_FFT, hop=HOP, win=WIN, compress=COMPRESS, length=None):
    ...
    return torch.istft(com, n_fft, hop_length=hop, win_length=win, window=window,
...
class MusicMPSENetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"clean": torch.from_numpy(clean / dp),
                "degraded": torch.from_numpy(degraded / dp)}
...
def train(epochs=100, batch_size=8, lr=3e-5, steps_per_epoch=200, resume=None):
    ...
    if not all_files:
        print(f"ERROR: No stem files in {musdb}"); return
