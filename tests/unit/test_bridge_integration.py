...
def test_bridge_calibration_data_structure():
    ...
    required = [
        'restorability_score', 'transfer_chain_depth', 'material_type',
        'snr_db', 'bandwidth_hz', 'chain_factor', 'artifact_freedom_min',
        'regression_threshold', 'quality_color', 'deep_chain_warning',
        'expected_phase_count', 'expected_duration_factor',
    ]
    for key in required:
...
@pytest.mark.parametrize("depth,expected_color,expected_af_min", [
    (1, "#2196F3", 0.95),
    (2, "#2196F3", 0.88),
    (3, "#4CAF50", 0.80),
    (4, "#E6A817", 0.70),
...
def test_bridge_data_depth_adaptive(depth, expected_color, expected_af_min):
    ...
    assert d['quality_color'] == expected_color, \
        f"depth={depth}: expected {expected_color}, got {d['quality_color']}"
