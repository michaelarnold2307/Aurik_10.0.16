...
def test_minimum_phase_delay_below_jnd():
    ...
    assert delay_ms < JND_GROUP_DELAY_MS, f"Min-phase delay {delay_ms:.2f}ms ≥ JND {JND_GROUP_DELAY_MS}ms — wäre hörbar"
...
def test_zero_phase_pre_ringing_at_audibility_threshold():
    ...
    assert 0.5 <= pre_ring_ms <= 5.0, f"Pre-ringing {pre_ring_ms:.1f}ms außerhalb plausibler Range [0.5, 5.0]ms"
...
def test_gdd_threshold_in_perceptual_range(depth, min_ms, max_ms):
    ...
    assert min_ms <= gdd <= max_ms, f"depth={depth}: GDD={gdd:.1f}ms außerhalb [{min_ms}, {max_ms}]ms"
...
def test_gdd_monotonic_with_depth():
    ...
    for depth in range(1, 6):
        ...
        prev = gdd  # type: ignore[assignment]
...
def test_regression_threshold_in_plausible_range():
    ...
    for depth in range(1, 6):
        for rs in [30, 50, 70, 90]:
            ...
            assert 0.01 <= t <= 0.10, f"depth={depth} rs={rs}: PMGG={t:.4f} außerhalb [0.01, 0.10]"
