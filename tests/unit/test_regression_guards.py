...
class TestGrooveGuard:
    ...
    def test_groovy_signal_retains_peaks(self):
        ...
        assert output_peak > 0.4, \
            f"§v10.112 Groove-Guard failed: peak={output_peak:.3f} (transients destroyed)"
    ...
    def test_already_natural_not_degraded(self):
        ...
        assert result.delta_hpe >= -0.05, \
            f"NaturalnessOptimizer degraded audio: delta_hpe={result.delta_hpe:.3f}"
...
class TestExcellenceCalibration:
    ...
    def test_auto_profile_uses_new_defaults(self):
        ...
        from backend.core.excellence_optimizer import MATERIAL_PROFILES, _MODULATION_STRENGTH, _HARM_BOOST_DB
        profile = MATERIAL_PROFILES['auto']
        assert profile.modulation_strength >= 0.50, \
            f"Auto modulation_strength too low: {profile.modulation_strength}"
        assert profile.harm_boost_db >= 3.0, \
            f"Auto harm_boost_db too low: {profile.harm_boost_db}"
    ...
    def test_shellac_profile_uses_high_values(self):
        ...
        assert profile.harm_boost_db >= 3.2, \
            f"Shellac harm_boost_db too low: {profile.harm_boost_db}"
...
class TestMaterialProfiles:
    ...
    def test_material_profiles_proportional(self):
        ...
        assert shellac.modulation_strength > cd.modulation_strength, \
            "Shellac sollte mehr Modulation brauchen als CD"
        assert shellac.harm_boost_db > cd.harm_boost_db, \
            "Shellac sollte mehr Harmonik-Boost brauchen als CD"
...
class TestPipelineEdgeCases:
    ...
    def test_clipping_through_optimizer(self):
        ...
        assert np.max(np.abs(result.audio)) <= 2.0, f"Output exceeded safety ceiling: {np.max(np.abs(result.audio)):.3f}"
...
class TestVocalQuality:
    ...
    def test_vocal_like_signal_retains_formants_after_non_vocal_phase(self):
        ...
        assert fs > 0.80, \
            f"§v10.117: EQ-Phase verschob Formanten (stability={fs:.3f}) — Gesang klingt unnatürlich"
    ...
    def test_breath_like_signal_survives(self):
        ...
        assert out_rms > in_rms * 0.01, \
            f"Breath completely removed: RMS ratio={out_rms/in_rms:.6f}"
