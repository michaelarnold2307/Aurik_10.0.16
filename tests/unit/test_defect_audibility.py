...
class TestDefectAudibility(unittest.TestCase):
    ...
    def test_01_all_thresholds_defined(self):
        ...
        required = [
            "clicks", "pops", "crackle", "dropout", "transport_bump",
            "hum", "hiss", "rumble", "noise_level", "surface_noise",
            "sibilance", "vocal_harshness", "stereo_imbalance",
            "phase_issues", "dc_offset",
            "bandwidth_loss", "pre_echo", "aliasing",
            "quantization_noise", "compression_artifacts",
        ]
        for dt in required:
            self.assertIn(dt, DefectScanner.AUDIBILITY_THRESHOLDS,
                          f"Missing threshold for {dt}")
    ...
    def test_03_benign_defects_masked(self):
        ...
        self.assertFalse(
            DefectScanner.is_audible("dc_offset", 0.1, "cd", -20, 50),
            "DC offset 0.1 should be inaudible"
        )
        # But at severity 0.99 → -0.5 dB, nearly full scale
        self.assertTrue(
            DefectScanner.is_audible("dc_offset", 0.99, "cd", -20, 60),
            "DC offset 0.95 should be audible"
        )
    ...
    def test_04_material_masking(self):
        ...
        self.assertFalse(
            DefectScanner.is_audible("clicks", 0.5, "dat", -10, 3000),
            "Clicks on DAT should be inaudible (no masking)"
        )
    ...
    def test_05_signal_masking(self):
        ...
        # -45 < -33 → NOT audible
        self.assertFalse(
            DefectScanner.is_audible("hiss", 0.25, "tape", -40, 8000),
            "Hiss in quiet signal should be masked"
        )
        # Loud (-10 dB RMS): signal_mask = 0.83, effective = -38 * (1-0.4*0.83) = -25.4
        # -45 < -25.4 → NOT audible (loud signal masks more)
        self.assertFalse(
            DefectScanner.is_audible("hiss", 0.25, "tape", -10, 8000),
            "Hiss in loud signal should be more masked"
        )
    ...
    def test_06_frequency_weighting(self):
        ...
        self.assertFalse(
            DefectScanner.is_audible("hum", 0.3, "tape", -20, 50),
            "Hum at 50Hz sev=0.3 should be inaudible"
        )
        ...
        self.assertTrue(audible_3000hz_mod,
                        "Hum at 3kHz sev=0.6 should be audible")
        self.assertTrue(audible_50hz_high,
                         "Hum at 50Hz sev=0.96 IS audible (just barely)")
    ...
    def test_08_all_materials_have_factors(self):
        ...
        std_materials = {"shellac", "vinyl", "tape", "cassette",
                         "reel_tape", "cd", "digital", "mp3_low", "dat"}
        for dt, (_, _, factors) in DefectScanner.AUDIBILITY_THRESHOLDS.items():
            missing = std_materials - set(factors.keys())
            self.assertEqual(len(missing), 0,
                             f"{dt} missing material factors: {missing}")
