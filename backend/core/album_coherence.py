...
class AlbumProfile:
    ...
    def update_from_track(self, lufs: float, lra: float, eq_curve: list[float],
                          dyn_range: float, quality: float) -> None:
        n = self.track_count
        w_old = n / (n + 1); w_new = 1.0 / (n + 1)
    ...
    def from_dict(cls, d: dict) -> AlbumProfile:
        p = cls(d["album_id"]); p.__dict__.update(d); return p
...
class AlbumCoherence:
    ...
    def apply_coherence(self, album_id: str, audio: np.ndarray, sample_rate: int,
                         current_lufs: float, track_quality: float) -> np.ndarray:
        ...
        logger.info("Album-Coherence [%s]: Track %d, LUFS-Gain %.1fdB, Ø-Qualität %.1f",
                     album_id, profile.track_count + 1, gain_db,
